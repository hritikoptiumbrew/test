...
class MoveSinglePageToMultiPageSubCategory implements ShouldQueue
{
     ...
     * @param $image_id
     */
    public function __construct($sub_category_id = NULL, $catalog_id = NULL, $image_id = NULL)
    {
    ...
    public function handle()
    {
        try {
                ...
                if (!$this->sub_category_id) {
                    if ($this->catalog_id) {
                        ...
                        if ($result) {
                            ...
                            Log::error("MoveSinglePageToMultiPageSubCategory : sub_category_id not found for catalog.", ["catalog_id" => $this->catalog_id]);
                            return;
                        }
                    } else {
                        ...
                        if ($result) {
                            ...
                            Log::error("MoveSinglePageToMultiPageSubCategory : sub_category_id not found for image.", ["image_id" => $this->image_id]);
                            return;
                        }
                    }
                }

                if ($this->sub_category_id == config('constant.SINGLE_PAGE_SUB_CATEGORY_ID') || $this->sub_category_id == config('constant.SINGLE_PAGE_SUB_CATEGORY_ID_FOR_LAYOUT')) {
                    ...
                    if ($this->sub_category_id == config('constant.SINGLE_PAGE_SUB_CATEGORY_ID')) {
                        ...
                        Log::error("MoveSinglePageToMultiPageSubCategory : Unidentified sub_category_id detected.", ["sub_category_id" => $this->sub_category_id]);
                        return;
                    }
                    ...
                    $updated_content = DB::select('SELECT
                                                   ...
                                                   WHERE
                                                      im.updated_at BETWEEN "' . $start_date . '" AND "' . $end_date . '" ', [$this->sub_category_id]);
            ...
            Log::error("MoveSinglePageToMultiPageSubCategory job handle() : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            DB::rollBack();
        }
    }
}
