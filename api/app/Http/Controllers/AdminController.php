            ...
            if (!Cache::has("pel:getCatalogBySubCategoryId$this->sub_category_id")) {
                $result = Cache::rememberforever("getCatalogBySubCategoryId$this->sub_category_id", function () {
                    ...
                    $result = DB::select('SELECT
                                        ...
                                        IF(ct.landscape_image != "",CONCAT("' . Config::get('constant.COMPRESSED_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . '",ct.landscape_image),"") as compressed_landscape_image,
                                        IF(ct.portrait_image != "",CONCAT("' . Config::get('constant.COMPRESSED_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . '",ct.portrait_image),"") as compressed_portrait_image,
    ...
    public function searchCatalogByName(Request $request_body)
    {
        try {
            ...
            $result = DB::select('SELECT
                                    ...
                                    IF(cm.landscape_image != "",CONCAT("' . Config::get('constant.COMPRESSED_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . '",cm.landscape_image),"") as compressed_landscape_image,
                                    IF(cm.portrait_image != "",CONCAT("' . Config::get('constant.COMPRESSED_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . '",cm.portrait_image),"") as compressed_portrait_image,
                                    IF(ct.icon != "",CONCAT("' . Config::get('constant.ORIGINAL_IMAGES_DIRECTORY_OF_DIGITAL_OCEAN') . '",ct.icon),"") as icon,
...
//                                      LIMIT ?,?', [$this->sub_category_id, $this->offset, $this->item_count]);
    ...
    public function addJson(Request $request_body)
    {
        ...
        try {
            ...
            $request = json_decode($request_body->input('request_data'));
            if (($response = (new VerificationController())->validateRequiredParameter(array('category_id', 'catalog_id', 'is_featured_catalog', 'is_featured', 'is_free'), $request)) != '')
                return $response;
    ...
    public function editJsonData(Request $request_body)
    {
        ...
        try {
            ...
            $request = json_decode($request_body->input('request_data'));
            if (($response = (new VerificationController())->validateRequiredParameter(array('category_id', 'is_featured_catalog', 'img_id', 'is_featured', 'is_free'), $request)) != '')
                return $response;
