...
//            $width = $img_h_w->width();
    ...
    */
    ...
    public function singleToMultipageScheduler(Request $request_body)
    {
        try {
            $request = json_decode($request_body->getContent());
            if (($response = (new VerificationController())->validateRequiredParameter(array('minutes'), $request)) != '')
                return $response;

            $this->minutes = $request->minutes;

            $start_date = date("Y-m-d H:i:s", strtotime($this->minutes));
            ...
            $updated_catalog = DB::select('SELECT
                                          ...
                                          FROM
                                              ...
                                              JOIN sub_category_catalog AS scc ON ctm.id = scc.catalog_id AND scc.sub_category_id = 157
                                          WHERE
                                              ctm.updated_at BETWEEN "' . $start_date . '" AND "' . $end_date . '" ');
            ...
            $updated_content = DB::select('SELECT
                                           ...
                                           FROM
                                              images as im
                                              JOIN sub_category_catalog AS scc ON im.catalog_id = scc.catalog_id AND scc.sub_category_id = 157
                                              JOIN catalog_master AS ctm ON ctm.id = scc.catalog_id AND ctm.is_featured = 1
                                           WHERE
                                              im.updated_at BETWEEN "' . $start_date . '" AND "' . $end_date . '" ');
            ...
            foreach ($updated_content as $content) {
                ...
                $new_catalog_id = $this->oldToNewCatalogId($content->catalog_id);
                if (!$new_catalog_id) {
                    $updated_catalog_data = DB::select('SELECT
                        ...
                        if($updated_catalog_data){
                            ...
                            DB::insert('INSERT INTO sub_category_catalog(sub_category_id,catalog_id,created_at) VALUES (?, ?, ?)', [158, $new_catalog_id, $create_at]);
                        }
                }
            ...
            $response = Response::json(array('code' => 200, 'message' => 'Schedule run successfully.', 'cause' => '', 'data' => json_decode("{}")));

        } catch (Exception $e) {
            Log::error("singleToMultipageScheduler : ", ["Exception" => $e->getMessage(), "\nTraceAsString" => $e->getTraceAsString()]);
            $response = Response::json(array('code' => 201, 'message' => Config::get('constant.EXCEPTION_ERROR') . 'get template detail.', 'cause' => $e->getMessage(), 'data' => json_decode("{}")));
        }
        return $response;
    }
